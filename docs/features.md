...
- **Growth banked between sessions is consumed silently.** The boot render still acknowledges a pending milestone (so it can't surface later as if it just happened) but shows no toast: `baselineGrowthStages()` only seeds `seenStage` when it's *absent*, so a category whose `totalHours` grew while the app was closed arrives at the first render with a real pending milestone. Only growth after `_sidebarBooted` is celebrated. Covered by the "pre-existing hours are baselined, not celebrated" regression in `scripts/test-growth.mjs`.
